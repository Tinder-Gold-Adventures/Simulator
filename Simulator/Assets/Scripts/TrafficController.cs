...
public class TrafficController : MonoBehaviour
{
    ...
    private List<TrainLight> trainlights; //Contains all train lights
    ...
    void Start()
    {
        ...
        trainlights = FindObjectsOfType<TrainLight>().ToList();
        ...
        //Add event listener to all sensors
    ...
    //Functions for setting a new state for a certain component
    ...
    private void SetTrainLightState(LaneTypes lanetype, int groupID, int componentID, BoatAndTrainLightState newState)
    {
        //Finds all warning lights with certain properties
        List<TrainLight> trainlightList = trainlights.FindAll(l => l.laneType == lanetype && l.GroupID == groupID && l.ComponentID == componentID);

        //No warning light found
        if (trainlightList == null)
        {
            Debug.LogError($"ERROR: Train_Light with groupID: {groupID} and componentID: {componentID} not found");
            return;
        }

        foreach (var light in trainlightList)
        {
            if (newState != light.state)
            {
                light.state = newState;
    ...
    //Creates a TopicInformation object from received topic
